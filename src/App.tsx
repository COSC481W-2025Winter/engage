...
function Home() {
  ...
  async function handleFollow() {
    ...
    try {
      const response = await axios.post(
        `${loginServer}/follow-user`,
        { fileName: fileName },
        { params: { auth: token }}
      );
      if (response.data.message.includes("User unfollowed successfully")) {
        // setFollowed(false);
        // setLikeCount((prev) => Math.max(0, prev - 1));
      } else if (response.data.message.includes("User followed successfully")){
        // setLiked(true);
        // setLikeCount((prev) => prev + 1);
      } else {
        alert("Follow error");
        console.error("Error following/unfollowing user:", response.data.message);
      }
    } catch (error) {
      // console.error("Error liking/unliking video:", error);
      // alert("Failed to process like. Please try again.");
